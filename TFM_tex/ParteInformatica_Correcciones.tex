...
En 2020, el \textbf{cáncer de próstata}, (\textit{Prostate Cancer, PCa}) \textcolor{red}{Yo no introduciría un acrónimo que solo vas a usar 2 veces contadas} fue el segundo tipo de cáncer más frecuente, y el quinto más mortal, en varones a nivel mundial \cite{GlobalCancer}. Un diagnóstico temprano de este es crucial para reducir la mortalidad del mismo. A día de hoy, el diagnóstico del PCa se basa en la realización de una \textbf{biopsia de próstata}\textcolor{red}{No se si pondría en negrita este detalle} en caso de sospecha, tras la evaluación inicial mediante pruebas como la medición del antígeno prostático específico o el examen rectal digital. Una biopsia de próstata es una prueba que consiste en la extracción de pequeños tejidos de la próstata para examinar posibles signos de cáncer (Ver Figura \ref{fig:biopsias_ejemplos}). El \textbf{Sistema de Puntuación de Gleason} (\textit{Gleason Score})\textcolor{red}{ Nuestro trabajo es en español, no debemos usar anglicismos además tan traducibles como lo has hecho (puntuación de Gleason vs Gleason Score)} es una calificación, en el rango de 2 a 10, que se da a biopsias de la próstata tras ser examinadas bajo un microscopio \cite{GleasonGov}. Valores más altos indican cánceres más agresivos y de crecimiento más rápido. En 2014 la \textbf{Sociedad Internacional de Patología Urológica} (\textit{International Society of Urological Pathology, ISUP}\textcolor{red}{Como estas siglas son una entidad, si está bien introducirla aunque sea para usarla una única vez}) propuso un nuevo sistema basado en la Puntuación de Gleason, que propone cinco grupos ordenados (\textbf{Gleason Groups, GGs}\textcolor{red}{Lo mismo, hace falta el acrónimo? Grupo de Gleason 1, Primer Grupo de Gleason...}) \cite{ISUP2014Disc}:
\begin{itemize}
...
\end{itemize}
\textcolor{red}{A mi me parece que la descripción de los grupos se puede hacer de manera más concisa, en un único párrafo propio y haciendo incapié a su caracter incremental, no pondría una lista como tal. }
...
\textcolor{red}{Aquí me falta un párrafo que justifique que el puntuar el grupo de Gleason es determinante para el diagnóstico y que se hace a mano por un experto, es costoso a nivel de tiempo de experto y puede ser subjetivo. Esto justifica el uso de IA para ayudar en la tarea.}

\textcolor{red}{El siguiente párrafo me parece que debe ir conectando en la linea anterior con que el uso de IA puede ayudar en la toma de decisiones pero, como comentas en el siguiente párrafo, en campos donde la decisión de la IA afecta a la vida de las personas, es necesario que la IA sea explicable, y ya entonces introduces XAI.}
...
\textcolor{red}{Una cosa que me corrijió muchas veces Julián en todos los trabajos que he hecho es poner \~ después de cada ref o cite, es decir, "...vease esta referencia~\cite{REVEL}} "



\section{Objetivos}

El objetivo principal del TFM es el uso y propuesta de técnicas de evaluación y mejora de explicaciones de modelos de clasificación de imágenes para detección de tejidos cancerosos en biopsias de próstata. Para el desarrollo del TFM, se divide el objetivo en distintos subobjetivos:
...
El trabajo requerido para realizar esta parte del TFG\textcolor{red}{Cuidado, es TFM no TFG} se ha dividido en cuatro fases:
\begin{itemize}
...
\textcolor{red}{Aquí deberías poner un pequeño repaso de los fundamentos teóricos necesarios para entender el trabajo, un párrafo que introduzca el capítulo.}

\section{Aprendizaje automático}

El \textbf{aprendizaje automático} (AA) o \textbf{machine learning} (ML) es una rama de la \textbf{inteligencia artificial}\textcolor{red}{ya has introducido IA, si la introduces, lo suyo es usarlo siempre} que se encarga del desarrollo de algoritmos que aprendan de datos o experiencias con el fin de mejorar su rendimiento en ciertas tareas \cite{alpaydin_introduction_2010-1, Mostafa2012, PatternRecogLibro, pml1Book}\textcolor{red}{Normalmente, cuando se introduce una cita, se introduce solamente una, si poner vartias seguidas es como que las pones por cumplir.}. Estos algoritmos disponen de un \textbf{modelo} definido por ciertos parámetros y dichos parámetros\textcolor{red}{..., los cuales serán los que...} serán los que se aprendan a través del \textbf{entrenamiento} o \textbf{aprendizaje}. \textcolor{red}{se debería unificar la terminología utilizada a lo largo del documento. Por ejemplo, si se introduce el término "aprendizaje automático", se debería utilizar de forma consistente en lugar de alternar con "machine learning". Recomiendo el uso de AA, ya que es el término en español y el trabajo está en español. entrenamiento o aprendizaje, elije uno y úsalo de forma consistente.}

Para saber a qué nos referimos cuando se habla de que el algoritmo \textit{``aprende''} conviene destacar la siguiente definición proveniente de \cite{mitchell_machine_1997}: \textit{``Un programa se dice que aprende de una experiencia E con respecto a una clase de tareas T y a una medida de rendimiento T, si su rendimiento en las tareas en T, medido por P, mejora con la experiencia E."} \textcolor{red}{Antes de usar el término de "aprender" en el párrafo anterior, la debes introducir como haces aquí. Pegaría dentro del párrafo anterior resumido }

El AA se utiliza habitualmente para resolver problemas complejos que no se pueden o se saben resolver con algoritmos tradicionales diseñados por humanos\textcolor{red}{sería más claro decir "que no se pueden abarcar fácilmente" o "para los cuales no se conoce una solución algorítmica directa". Los problemas además no se resuelven, se abarcan. Resolver es para cosas exactas, los problemas de AA como clasificación de imágenes las cuales no son exactas, se abarcan}. Además, se requiere tener datos de los que poder aprender\textcolor{red}{haría hincapié en el volumen de datos también}. Algunos de los campos donde se utilizan el AA para resolver este tipo de problemas son el procesamiento de lenguaje natural, el reconocimiento del habla o la visión por computador, entre muchos otros.

Los algoritmos de AA se clasifican en distintos paradigmas dependiendo de los datos o experiencia de la que se disponga. Existen tres paradigmas principales: el \textbf{aprendizaje supervisado}, el \textbf{aprendizaje no supervisado} y el \textbf{aprendizaje por refuerzo} \cite{Mostafa2012,Goodfellow-et-al-2016}\textcolor{red}{Lo dicho con las citas. Si las dos son importantes, puedes destacar cada cita individualmente. Si no, usa la más relevante nada mas}.

\textcolor{red}{Además, existen otros tipos de AA: Semi supervisado, autosupervisado, por transferencia, aprendizaje activo... Yo comentaría solamente el supoervisado, el no supervisado y la existencia de otros tipos, los cuales no son relevantes para este trabajo.}

El aprendizaje supervisado hace referencia a los algoritmos de AA en los que se tiene un conjunto de datos agrupados en pares dato-\textbf{etiqueta}. La etiqueta se corresponde con la salida que se requiere del algoritmo de AA cuando tenga de entrada dicho dato. Un ejemplo típico de conjunto de datos \textcolor{red}{Si hablas de MNIST, citalo} de este tipo sería un conjunto de imágenes de números escritos a mano y para cada imagen el número que está escrito en dicha imagen. \textcolor{red}{Yo también pondría una reflexión que viene del término con su cita: Se le llama supervisado porque la etiqueta supervisa la salida que debe tener el algoritmo.}

El aprendizaje no supervisado hace referencia \textcolor{red}{Usa otra palabra, ya has repetido hace referencia en este párrafo y en el anterior}a los algoritmos de AA en los que se disponen de datos que no están etiquetados. En este tipo de algoritmos no se quiere aprender algo específico de los datos sino más bien encontrar patrones o estructurar los datos de entrada. Por ejemplo se dispone de distinta información relativa a libros (autor, título, número de páginas, etc) y el algoritmo categoriza los libros en distintas categorías, aunque no se sepa a qué se corresponde cada categoría.
...
Para este TFG\textcolor{red}{Ciudado, es TFM no TFG} es relevante el aprendizaje supervisado. En particular serán relevantes los dos siguientes problemas típicos de aprendizaje supervisado.\textcolor{red}{ No es que sea relevante, es que es el enfoque que más se usa en el trabajo.}

\textcolor{red}{Resumen de las subsecciones que se vienen a continuación.}

\subsection{Problema de clasificación}

Este problema trata de clasificar las entradas de un tipo concreto en un número de clases $k$. Es decir\textcolor{red}{Normalmente, después de un primer párrafo introductorio al tema, me suele gustar que en el segundo sea donde se empieza con la formalización, y puedes poner directamente: Formalmente, dado $X\subset \mathbb{R}^n$ ... . Por tanto, intenta separa la formalización de la introducción al tema}, para una entrada el programa deberá clasificar dicha entrada en una de las $k$ clases. Por lo tanto el algoritmo produce una función $f:\mathbb{R}^n \rightarrow \{1,...,k\}$ que a cada entrada $\bx$ le asocie una clase $y = f(\bx)$. Dicha función tratará de aproximar una función objetivo $g$ que clasifique perfectamente todas las entradas.
...
Algunas formas de resolver estos problemas son mediante árboles de clasificación, máquinas de vectores-soporte o mediante redes neuronales. \textcolor{red}{Una frase para un párrafo es muy escueto. Por otra parte, aunque lo que dices es verdad, no vamos a hablar de SVMs ni árboles de decisión, no los incluyas.}

Un caso particular de este problema que es más relevante para este trabajo es el caso en el que hay dos clases. Este problema se denomina \textbf{problema de clasificación binaria}. En este TFG se tratará de determinar visibilidad de puntos de la cara de una persona en una imagen luego las dos clases se corresponden con visible o no visible.\textcolor{red}{TFG de nuevo. Pero además, lo de las caras que es?? Por otra parte, no vamos a hablar de clasificación binaria, nuestro problema también es multiclase¡¡. Además, no es 'relevante', es la manera en la que abordamos el problema. Podríamos utilizar mil formas de abordarlo, pero lo enfocamos desde un punto de vista de clasificación.}

\subsection{Problema de regresión}
Este problema es similar al de clasificación, pero la función que aprende el algoritmo es de la forma $f:\mathbb{R}^n \rightarrow\mathbb{R}^m$. Por lo tanto ahora el programa debe predecir un valor numérico (o varios si $m > 1$) para una entrada concreta. Al igual que ocurría en el problema de clasificación se disponen de una serie de datos de entrada y el resultado que se espera del programa para dichas entradas, es decir $\{\bx_1,...,\bx_N\}$ con $\bx_i\in\mathbb{R}^n$ y  $\{\by_1,...,\by_N\}$ con $\by_i \in \mathbb{R}^m$. \textcolor{red}{De echo suele ser alreves, lo que primero se introduce es el problema de regresión y, para el caso de redes neuronales para clasificación, se aplica la softmax y se adapta para clasificación multiclase. Cambiaría de orden.}

Algunas formas de resolver este tipo de problemas son mediante la regresión lineal, máquinas de vectores-soporte o redes neuronales.\textcolor{red}{Lo mismo, no aporta nada si no lo vamos a usar.}
...
\textcolor{red}{Aquí pega una formalización, los ejemplos después.}

Por ejemplo, una función de coste habitual es el error cuadrático medio\textcolor{red}{, definido como}:
...
\textcolor{red}{Para la minimización de la función de coste, se suele optimizar los parámetros del modelo utilizado el métodod de...} El método principal\textcolor{red}{principal queda raro} en el AA para minimizar dicha función es el \textbf{gradiente descendente}. Este método iterativo se basa en que, para una función multivariable dos veces diferenciable, el gradiente de dicha función en ese punto (que es un vector)\textcolor{red}{aclaración innecesaria} apunta en la dirección contraria al ascenso más empinado\textcolor{red}{de mayor ascenso/aumento. empinado para las colinas en el campo, esto es un TFM}. Por lo tanto tomando pasos en la dirección contraria al gradiente, debería disminuir la función de coste y por lo tanto dar un mejor rendimiento.
...
Normalmente, \textcolor{red}{Este conector es poco científico.}el método utilizado para resolver el problema de AA hace que las funciones que se pueden representar en la solución estén parametrizadas por un vector de parámetros o \textbf{pesos}\textcolor{red}{muy repetitivo, parametrizadas - parametros. usa solo 'vector de pesos'} $\bw$.\textcolor{red}{Además, yo no hablaría de 'el método utilizado', ya que hay varios y varios utilizan esta perspectiva. (Para abarcar un problema con AA, se suelen simplificar las posibles configuraciones como un vector de pesos)} Por ejemplo, en un problema de regresión con función objetivo $f:\mathbb{R}^n \rightarrow \mathbb{R}$ que se plantea resolver mediante regresión lineal se tiene que la función que se aprende toma la forma: 
\begin{align*}
f_\bw(\bx) = \bw^\top \bx \quad \quad , \bx \in \mathbb{R}^n, \bw \in \mathbb{R}^n
\end{align*}

Donde el vector $\bw$ son los parámetros que se pueden cambiar a la función para mejorar su rendimiento. \textcolor{red}{Aquí diría también que otros ejemplos más complejos como las redes neuronales también se pueden ver como funciones parametrizadas.}

La función de coste dependerá de dichos parámetros. Luego, el gradiente descendiente\textcolor{red}{Gradiente descendente, no descendiente} cambiará el valor de los parámetros en cada paso buscando minimizar la función de coste. Estos cambiarán según la siguiente regla:
\begin{align*}
\bw_n = \bw - \eta \nabla E(\bw)
\end{align*} 
...
El algoritmo del gradiente descendente\textcolor{red}{corrección: descendente, no descendiente} inicia los pesos $\bw$ a un valor aleatorio y empieza a iterar con la regla del descenso, buscando reducir el error en cada paso durante un número de \textbf{épocas} \textcolor{red}{lo siguiente de la frase no lo pondría, e introducir que hay otros criterios de parada es liarnos con otros conceptos} (cada época itera sobre todos los datos de entrenamiento), aunque hay otros criterios de parada del algoritmo. En caso de ser satisfactorio\textcolor{red}{En caso de converger,} el algoritmo \textcolor{red}{se detiene} en un mínimo local del error de pérdida. \textcolor{red}{Se suele buscar además que la función de pérdida sea un una función convexa para asegurar además que este es un mínimo global.} 
...
\textcolor{red}{Quita el asterísco a los títulos, quedan mejor numerados}

Cuando se optimiza un modelo \text{de IA}, puede ocurrir que se ajuste a los datos de entrada con los que se ha entrenado, pero tenga mal rendimiento para datos {con los que no se ha entrenado}\textcolor{red}{que no haya visto anteriormente} y, por lo tanto, no generalice bien lo aprendido de los datos de entrenamiento al problema. Cuando esto ocurre, \textcolor{red}{las comas después de los conectores} se dice que el modelo se ha \textbf{sobreentrenado} o \textbf{sobreajustado}. Una forma de identificar \textcolor{red}{el sobreajuste} es guardándonos\textcolor{red}{reservando o separando} un {número}\textcolor{red}{porcentaje} de datos con los que no se va a entrenar el modelo y comparar el error del modelo a lo largo del entrenamiento de los datos de entrenamiento con los que nos\textcolor{red}{no} se han guardado. El sobreentrenamiento se puede observar cuando el error en los datos con los que no se entrena empieza a aumentar a medida que se sigue entrenando el modelo como se puede ver en la Figura \ref{fig:sobreentrenamiento}.
...
\textbf{Deep learning}\textcolor{red}{Pondría un espacio antes del paréntesis}(DL \textcolor{red}{Aprendizaje profundo es la traducción, elige si quieres que todas las siglas sean en inglés o en español, pero solo uno}) \cite{DeepLearningFoundationsConcepts, Goodfellow-et-al-2016, LeCun-Yann-Bengio, UnderstandingDeepLearning, Schmidhuber_2015}\textcolor{red}{Otra vez, muchas citas seguidas. Escoge la más relevante o más reciente} es un subcampo del AA determinado por los métodos con los que aprende y\textcolor{red}{el} tipo de datos con los que trata. Los algoritmos de DL pueden tratar datos poco estructurados como pueden ser imágenes o textos. Este tipo de algoritmos automatizan la extracción de características, es decir, determinan qué información de los datos es relevante para la resolución del problema. Esto elimina mucha de la intervención necesaria por los humanos\textcolor{red}{parte de la intervención humana necesaria} a la hora de crear el algoritmo.
...
\subsection*{Redes neuronales \textcolor{red}{artificiales. Cambia el resto y lo mismo, a español (RNA)}}
Una \textbf{red neuronal} (\textit{artificial neural network}, ANN) \cite{NNPatternRecogLibro,Haykin,NNTricks}\textcolor{red}{Otra vez muchas citas. Escoge la más representativa} es un modelo de AA que consiste en una red conectada de \textbf{nodos} o \textbf{neuronas}. Cada neurona recibe una serie de números reales (señales)\textcolor{red}{entradas} de las neuronas conectadas a esta realiza una operación que resulta en una salida y la manda a los nodos conectados a esta. 
...
\textcolor{red}{Hay un error en la fórmula. Debería ser $y = \sigma(\sum_{i=1}^{n}w_i x_i + w_0)$. La función de activación se aplica al resultado de la suma ponderada, no dentro de la suma.}

Dónde\textcolor{red}{Donde (sin tilde)} $x_i$ son las entradas de la neurona, $w_i$ son los \textbf{pesos} de la neurona, en particular a $w_0$ se le denomina sesgo\textcolor{red}{bias}, y $\sigma$ es una función de los números reales a los números reales y se le denomina \textbf{función de activación} \cite{article_func_activacion}. Al conjunto de los pesos de todas las neuronas de una red se les denomina, naturalmente, \textbf{pesos de la red}. A través de la modificación de dichos pesos es como se entrenará a una red para realizar una tarea concreta.
...
\caption{ Ejemplo de ANN con una única capa oculta y con dos salidas. Imagen extraída de \protect\cite{Haykin}.}
\label{fig:red_neuronal_ff}
\end{figure}

Si se considera ahora la función $f$ que implementa un\textcolor{red}{una} red MLP, se tiene que esta depende de los pesos de la red. Una vez se tengan unos datos con los que entrenar la red y una función de coste que evalúe el rendimiento de la red, se puede aplicar el gradiente descendente\textcolor{red}{descendente, no descendiente} estocástico para entrenar la red modificando los pesos de esta\textcolor{red}{de esta → de la misma o de la red}. Sin embargo, el cálculo del gradiente de la función de error en un\textcolor{red}{una} red puede resultar muy costoso, especialmente para el cálculo de las derivadas parciales del error respecto de los pesos de las neuronas en las capas menos profundas\textcolor{red}{más alejadas de la salida o más cercanas a la entrada}. Es por esto que se suele utilizar un algoritmo, denominado \textbf{backpropagation}, para el cálculo del gradiente. 
...
\textbf{Batch normalization} \cite{ioffe2015batch} es una\textcolor{red}{un} método consistente en normalizar los datos de entrada de cada capa. De esta manera se consigue independizar\textcolor{red}{independizar suena raro, mejor: hacer más independiente} la distribución de los datos de entrada de cada capa de la red, con\textcolor{red}{de} los parámetros aprendidos por la red. Está probado que este método hace que el entrenamiento de la red sea más estable y más rápido. Además también se sabe que tiene un efecto regularizador en la red, lo que disminuye la probabilidad de que la red sobreentrene.
...
Este operador no es más que la adaptación de la convolución al caso en el que $f$ y $g$ sean funciones discretas, es decir, definidas sobre los números enteros, $\mathbb{Z}$. Este es el operador que aplican las capas convolucionales a sus entradas. En el caso de las convoluciones aplicadas en una CNN $f$ se corresponde con la entrada de la capa convolucional y $g$ con el \textbf{núcleo}, \textbf{kernel} o \textbf{filtro}\textcolor{red}{Quita el punto antes de 'de la convolución'}. de la convolución. El \textit{kernel} no es más que una matriz de dimensiones $n\times m$ de valores reales. Véase la Figura \ref{fig:ejemplo_convolucion} para ver un ejemplo de una convolución.
...
Usualmente, la entrada de las capas convolucionales tienen\textcolor{red}{tiene (singular)} más de un canal (por ejemplo la capa inicial suele tener 3 canales, uno por cada color primario), en tal caso, el \textit{kernel} tiene tantos canales como la entrada y aplica cada uno a su canal correspondiente, sumando la salida\textcolor{red}{sumando las salidas}. Para un ejemplo ver la Figura \ref{fig:ejemplo_convolucion_3canales}. A la salida de una convolución se le llama \textbf{mapa de características}. 
...
\caption{En la figura se ver\textcolor{red}{ve} cómo se aplica \textit{max pooling} y \textit{average pooling} a una entrada. Como las ventanas tienen tamaño $2\times 2$ se dice que se aplica \textit{pooling} $2\times 2$. Imagen extraída de \url{https://saturncloud.io/blog/a-comprehensive-guide-to-convolutional-neural-networks-the-eli5-way/}.}
\label{fig:ejemplo_pooling}
\end{figure}

\subsection*{Capas totalmente conectadas}
Este tipo de capas no son más que una capa típica de MLP que conecta todos los valores de las neuronas de la capa anterior con todas las neuronas de esta capa. Se colocan después de todas las capas convolucionales\textcolor{red}{y} de \textit{pooling}. Por ejemplo, si se quisiera resolver un problema de clasificación de imágenes en cinco clases, se tendría una última capa totalmente conectada con cinco neuronas.
...
Por otro lado los modelos de caja negra son modelos difíciles de interpretar y explicar debido a que tiene\textcolor{red}{tienen} estructuras complejas, con una gran cantidad de parámetros y no lineales\textcolor{red}{y comportamiento no lineal}. Los modelos más importantes pertenecientes a esta clase son las ANNs. 
...
La \textbf{Inteligencia Artificial eXplicable} (\textit{eXplainable Artificial Inteligence, XAI}) \cite{XAIManifesto, XAIWHATWEKNOW, XAINOTIONS, XAICONCEPTS}\textcolor{red}{Otra vez muchas citas seguidas. Escoge una o dos más relevantes y di algo distinto para cada una de ellas} es un campo de estudio que pretende desarrollar métodos para construir modelos de ML que aumenten la interpretabilidad de estos sin que esto afecte demasiado a su rendimiento. Para ello hay\textcolor{red}{existen} caminos\textcolor{red}{dos enfoques principales}:
...
\end{itemize}
\textcolor{red}{Itemize demasiado escueto y con solo dos elementos. Describe mínimamente cada uno en el párrafo anterior y esribe un párrafo para cada una de las dos ideas.}

En XAI, la \textbf{Interpretabilidad} se refiere a la capacidad de comprender cómo los modelos de IA toman sus decisiones. Por otro lado\textcolor{red}{,} la \textbf{explicabiliad}\textcolor{red}{explicabilidad} se refiere a la capacidad de hacer interpretaciones\textcolor{red}{no uses el verbo interpretar para explicar 'explicabilidad, puede dar lugar a confusiones.} y describir el funcionamiento interno de un sistema de IA en términos humanos. Estos dos son los dos principales criterios a la hora de evaluar métodos de XAI, además de ellos existen otros como \textbf{transparencia}, \textbf{justicia} y \textbf{robustez}. 

En general, el objetivo de la explicabilidad es hacer algún aspecto del modelo entendible para el usuario. Si lo que se intenta hacer expllicable\textcolor{red}{explicable} es una salida del modelo para una instancia concreta, se habla de \textbf{explicabilidad local}. En cambio\textcolor{red}{No es en cambio, si no si esa explicación además es capaz de explicar el comportamiento completo}, si es el dataset entero se habla de \textbf{explicabilidad global}.

Una distinción en los métodos de XAI es si el método es \textit{\textbf{ante-hoc}} o \textit{\textbf{post-hoc}}. Los métodos \textit{ante-hoc} son aquellos que entrenan un método\textcolor{red}{modelo con el objetivo de la explicabilidad impuesto de antemano} explicable en primer lugar. \textcolor{red}{Un ejemplo de ello son los árboles de decisión, ya que siguiendo los nodos de sus ramas se puede extraer el razonamiento intrínseco del modelo}. Por otro lado los \textit{post-hoc} pretenden explicar un modelo de caja negra después de ser explicado\textcolor{red}{después de haber sido entrenado. Un ejemplo son las redes neuronales, las cuales no están pensadas para extraer un razonamiento humano de manera directa}. 

\textcolor{red}{Te he separado los párrafos. Extiende este de la misma manera que el anterior}A su vez, los métodos \textit{post-hoc} se dividen en métodos \textbf{agnósticos al modelo} si funcionan para cualquier modelo, y métodos \textbf{específicos al modelo} si sólo funcionan para un tipo (o clase) de modelos.

Por último, está la siguiente distinción de métodos según la metodología que utilicen: \textbf{basados en retropropagación} y \textbf{basados en perturbaciones}. Los que se basan en retropropagación propagan una señal de la salida del modelo hacia la entrada. Los basados en perturbaciones modifican la entrada del modelo utilizando distintas técnicas para investigar el impacto de estas modificaciones en la salida del modelo. \textcolor{red}{Aquí quieres hacer una distinción entre CRP y LIME entiendo. Sin embargo, esta distinción ya viene dada en la diferenciación entre agnósticos y específicos al modelo. Quitaría este párrafo}
...
Las \textbf{Explicaciones lineales locales} (\textit{Linear Local Explanation, LLE}) \cite{REVEL,LIME} son una clase de explicaciones para modelos de caja negra.
 ...
 Sea $X \subset \mathbb{R}^F$ el datatset\textcolor{red}{conjunto de datos} de entrada, siendo $F$ el número de características por ejemplo, y sea $f : \mathbb{R}^F \rightarrow \mathbb{R}^C$ la función que implementa un modelo de caja negra, dónde\textcolor{red}{donde} $C$ es el número de clases de un problema de clasificación al que se aplica. Sea $x\in X$ una entrada a ser explicada. Un explicador LLE de caja blanca es una función $g: \mathbb{R}^F \rightarrow \mathbb{R}^C$ definida como:
 
\begin{equation}
g(x) = Ax + B, \quad A \in \mathcal{M}_{F,C}, B \in \mathbb{R}^C
\end{equation}

Dónde\textcolor{red}{Donde} $\mathcal{M}_{F,C}$ es el conjunto de todas las matrices con $F$ filas y $C$ columnas. Es decir, $g$ no es más que una función lineal que lleva una entrada del modelo al espacio de salida del modelo. Este modelo se puede interpretar pues intuitivamente para cada peso $a_{i,j}$ de $A$ denota la importancia que tiene la característica de entrada $i$ con\textcolor{red}{para} la predicción de la salida $j$. Por otro lado cada valor $b_j$ del vector $B$ está conectado con la importancia en general de la salida $j$.
...
A partir de esta matriz se define la \textbf{matriz de importancia normalizada} $A_r=\frac{A}{ max_{A_{i,j}}\{|A_{i,j}|\} }$ que lleva los valores de $A$ al rango $[-1,1]$ manteniendo su signo. Finalmente se define la \textbf{matriz de importancia absoluta} como la matriz $|A|$ que simplemente es la matriz $A_r$ con todos sus elementos en positivo. \textcolor{red}{Donde cada elemento $a_{i,j}$ es el valor absoluto de $a_{i,j}$ de $A_r$.}

\highlight{Leyendo tu artículo no me queda claro si la matriz de importancia absoluta es $|A_z|$ o $|A|$}
\textcolor{red}{Si la confusión es si considero $A^l$ logits y $A^p$ probabilities, la matriz de importancia es la combinación de ambas. Le cambio el nombre a $\mathcal{A}$, no a una simple $A$. Defino varias porque cada uno sirve para una cosa distinta. Para concision, la matriz de importancia relativa absoluta, es decir, $|\mathcal{A}_r|$}. Para concordance y fidelity, la matriz de importancia es $\mathcal{A}_r$, aunque se puede usar indistintamente $\mathcal{A}$, pues es el mismo orden.
...
\textcolor{red}{Las comas se deben de introducir bastante más, es un respiro para el lector. Por ejemplo, en la frase anterior te lo he cambiado: De aquí en adelante,(coma)... . Repasa en el texto, o pidele a algún chat como copilot, chatgpt o cluade que te ofrezca cambios y los repasas.}

\subsubsection*{Local concordance}
\textcolor{red}{Siempre que hables de una introducción de un término o una propuesta, primero debes poner un párrafo con la idea intuitiva de la propuesta, y luego la definición matemática. Esto vale para todas las métricas.}

\textcolor{red}{Formalmente, dados el modelo $f$ y la explicación $g$ para una instancia $x$, podemos definir...}Se define como:

\begin{equation}
Local\_Concordance(g) =  1 - \frac{||f(x)-g(x)||}{C'}\textcolor{red}{como es la continuación de un frase que es una ecuación, se pone coma al final},
\end{equation}

\textcolor{red}{y se continua la frase con minúscula: d}ónde $||\cdot||$ es una norma arbitraria (por ejemplo la norma euclídiana $||\cdot||_2$) y $C'$ es la distancia máxima posible entre dos vectores de probabilidad (por ejemplo\textcolor{red}{ejemplo de coma:,} en el caso de $||\cdot||_2$ se cumple que $C'=\sqrt{2}$). 
...
\highlight{Aquí he adaptado la fórmula de \href{tu artículo}{https://arxiv.org/abs/2211.06154}\textcolor{red}{Okey pero mensiona el que está publicado, no el arxiv jaja} con la información de la documentación de tu código en github para que quede más claro (a mi me queda más claro).} 
\textcolor{red}{Perfecto, sin problema}
...
\subsubsection*{Robustness over explanations\textcolor{red}{Español}}
...
La \textbf{regularización} \cite{XSHIELD, Regularization}\textcolor{red}{Es una regularización, no 'la'} es la modificación de la arquitectura del modelo\textcolor{red}{No se modifica la arquitectura, se modifican los pesos}, del proceso de aprendizaje o inferencia para imponer algún criterio \textcolor{red}{de calidad}sobre este. Por ejemplo, dropout, que ya se vió previamente, es una regularización.\textcolor{red}{No hace falta esta frase, ya lo has dicho. Igualmente, no hace falta que aclares qué es una regularizaciñon de nuevo si ya lo has dicho. En tal caso, este párrafo debe ir más en la linea de 'Como ya se ha visto, existen técnicas de regularización como dropout que...' }

En particular nos interesan las regularizaciones que implican añadir a la función de coste un nuevo término que indica el criterio que queremos minimizar en el modelo. Algunos ejemplos típicos son la regularización L1 o L2. En general\textcolor{red}{coma} se tiene que una regularización de este tipo se expresa como:

\textcolor{red}{Te propongo que primero hables de las regularizaciones generales y su definición. En párrafos posteriores, introduces 'lo particular' que te interesa.}
...
En primer lugar, presentamos la familia de regularizaciones \textbf{\textit{Transformation-Selective Hidden Input Evaluation for Learning} ,T-SHIELD} \cite{XSHIELD}. Esta está diseñada para hacer que el modelo aprenda con menos features. Para ello se ocluye\textcolor{red}{ oculta} parte de la entrada. Formalmente, se utiliza una transformación $T: \mathbb{R}^F \rightarrow \mathbb{R}^F$ tal que $T(x)_i = x_0$ si $i$ es una característica a ocultar y $T(x)_i = x_i$ si no lo es. Es decir, está transformación cambia todas las características de una entrada $x$ por un valor constante $x_0$ y deja el resto intactos.

Para una transformación $T$ como la anterior descrita\textcolor{red}{ descrita anteriomente}, se define la regularización T-SHIELD como sigue:

\begin{equation}
T-SHIELD(x,\Theta) = KL(f_\Theta(T(x)),f_\Theta(x)) + KL(f_\Theta(x),f_\Theta(T(x))) 
\end{equation}

Dónde $KL(\cdot,\cdot)$ denota la distancia\textcolor{red}{ES UNA DIVERGENCIA, NO UNA DISTANCIA. A otro estudiante de TFG le sacaron los colores por decir esto} de Kullback-Leibler, que se trata de un tipo de distancia que mide como difieren dos distribuciones distintas. Por lo tanto, esta regularización no es más que la distancia de Kullback-Leibler simétrica. Según como se construya la transformación $T$ esta regularización penalizará ciertos aspectos del modelo. En la Figura \ref{fig:tshield_flow} se puede ver cómo funcionaría una regularización de esta familia de forma general.

\begin{figure}[h]
\noindent
\makebox[\textwidth]{\includegraphics[scale=0.5]{images/tshield_flow}}
\caption{ En esta figura se puede ver cómo funciona en general la familia de regularizaciones T-SHIELD durante el entrenamiento. En gris se puede ver el flujo normal de la imagen en el modelo. En azul se encontraría la forma en la que la regularización decide qué características ocluir. Finalmente en rojo se encontraría el flujo de la imagen con las características ya ocluidas y cómo se calcula la función de regularización. Imagen extraída de \protect\cite{REVEL}.}
...
\caption{ Comparación entre el comportamiento de $T$ en R-SHIELD y X-SHIELD. La imagen está sacada del dataset \textit{Flowers} de clasificación de imágenes de flores. Se puede ver que R-SHIELD ocluye partes de la imagen aleatorias, incluyendo partes de la flor a clasificar. Por otro lado X-SHIELD ocluye partes del fondo de la imagen que intuitivamente no parecen útiles para clasificar dicha flor. Imagen extraída de \protect\cite{REVEL}.}
\label{fig:rshield_vs_xshield}
\end{figure}


Se denomina \textbf{XAI-SHIELD (X-SHIELD)} a otra regularización de la familia T-SHIELD. Comparte el parámetro $\lambda$ de R-SHIELD pero en vez de ocultar el $\lambda\%$ de características aleatorias, va a ocultar las $\lambda\%$ menos importantes. Para ello, a cada característica le otorga la importancia $||v_i||$ donde $v_i$ es la fila $i$ de la matriz de importancia $A$ introducida en la subsección \ref{subsec:revel}, pero para el modelo que se está entrenando. \textcolor{red}{En lugar de decir la diferencia, di primero qué es X-SHIELD, sin comparativas. Luego en otro párrafo haces la comparación con R-SHIELD.}
...
Este Trabajo de Fin de Máster trata de resolver \textcolor{red}{abarcar} el problema de desarrollar un modelo de IA de clasificación para lesiones cancerosas en biposias \textcolor{red}{biopsias (error ortográfico)} de próstata y de la mejora en interpretabilidad\textcolor{red}{explicabilidad} del modelo mediante técnicas de XAI. Por lo tanto vamos a hacer una revisión del estado del arte para los cuatro siguientes puntos \textcolor{red}{que son relevantes (frase incompleta)}:
...
\item Métodos de XAI de regularización para mejoras \textcolor{red}{mejorar} las explicaciones.
\item Métricas de XAI de \textcolor{red}{para evaluar la} calidad de las explicaciones producidas para poder comparar los distintos métodos utilizados.
\end{itemize}

\subsection*{Modelos de clasificación de imágenes}

\textbf{Efficientnet} \cite{efficientnet2019} es una red introducida en 2019 que se sigue considerando estado del arte en clasificación de imágenes cuando se mide eficiencia computacional, es decir, rendimiento obtenido \textcolor{red}{por} número de parámetros. En la Figura \ref{fig:efficientnet_grafica} se puede apreciar como es más eficiente en cómputo respecto \textcolor{red}{a} otros modelos de clasificación de imágenes \textcolor{red}{como} ResNet \cite{resnet} o DenseNet \cite{densenet} para el dataset Imagenet. \textcolor{red}{Esta sección debería estar en el capítulo de Métodos, no en Estado del Arte.} 

Existen otros modelos más modernos como CoCa \cite{coca}, DaViT \cite{DaViT} o ConvNeXt V2 \cite{ConvNext}, que utilizan transformadores\textcolor{red}{transformers} en el caso de \textcolor{red}{CoCa} y DaViT, o autoencoders, en el caso de ConvNeXt V2. Estos modelos alcanzan precisiones mayores que EfficientNet\textcolor{red}{.} Sin embargo\textcolor{red}{,} aumentan demasiado el número de parámetros utilizado \textcolor{red}{como} para que sea una propuesta viable para nuestro trabajo.

Por ello, hemos decidido utilizar el modelo EfficientNet en este trabajo ya que ofrece un balance muy bueno entre número de parámetros y precisión. En la Tabla \ref{tab:comparacion_modelos_efficientnet} se puede apreciar las diferencias entre el modelo que utilizaremos\textcolor{red}{,} EfficientNet-B2\textcolor{red}{,} y las versiones más pequeñas de los modelos \textcolor{red}{mencionados}. Se aprecia que EfficientNet es más eficiente en el número de parámetros. \textcolor{red}{Esta justificación es muy débil. Deberías incluir más detalles sobre limitaciones computacionales específicas y por qué estos otros modelos no son viables.}
...
Una revisión de todos los métodos de explicabilidad \textit{post-hoc}, que son los que se aplican a modelos de caja negra, está fuera de\textcolor{red}{l} \textcolor{red}{del} alcance de este trabajo \textcolor{red}{lo que resta de la frase simplemente no lo pondría}pues sería inabarcable. Sin embargo\textcolor{red}{,} resumimos algunos métodos populares (esquema en la Tabla \ref{tab:xai_explicaciones}): \textcolor{red}{Esta introducción es muy vaga. Deberías explicar por qué seleccionas específicamente estos métodos y cómo se relacionan con tu trabajo. 'Los métodos escogidos son post-hoc para evaluación, LRP y CRP para visualización y Grad-cam e IG para la construcción de regularizaciones.' Desarrolla un poco más esa idea}
...
En este estudio nos hemos dejado muchos métodos como explicaciones por ejemplos (como generación de \textit{counterfactuals} \cite{counterfactuals}) o las muchas extensiones de los métodos \textcolor{red}{mencionados} (por ejemplo para LIME en \cite{LIMESURVEY} hay una taxonomía de \textcolor{red}{éstos}). \textcolor{red}{Esta frase es confusa y suena como una disculpa. Mejor explica por qué estos métodos específicos son relevantes para tu trabajo y cómo conectan con las métricas REVEL que usarás.}


\subsection*{Métricas para explicaciones}

Debido a que hay muchos tipos de explicaciones en la literatura (LLEs, mapas de atribución, reglas...) no hay manera de comparar estas directamente. Debido a esto, \textcolor{red}{existen} distintas métricas para medir de forma cuantitativa la calidad de las explicaciones. \textcolor{red}{Esta introducción es redundante ("debido a esto... debido a esto"). Reescribe de forma más clara y directa.}

En \cite{metricasReglas}, se proponen cuatro métricas para medir la calidad de explicaciones. Sin embargo\textcolor{red}{,} estas son específicas para explicaciones basadas en reglas. Las métricas que \textcolor{red}{proponen} son: \textcolor{red}{Esta transición es abrupta. Deberías conectar mejor por qué mencionas estas métricas si son específicas para reglas cuando tu trabajo es sobre LLEs.}
\begin{itemize}
...
\item $R$: El número de reglas de la explicación. Busca cuantificar lo simple que es la explicación. \textcolor{red}{¿Cuán simple es la explicación? Revisar redacción: es decir, el número de reglas o variables que utiliza.}
\item $F$: El número de características que se han utilizado para construir la explicación. Es similar a $R$\textcolor{red}{;} cuanto menor es\textcolor{red}{,} mejor se considera la explicación.
\item $S$: La estabilidad de la explicación. Cuantifica la habilidad de la explicación de manejar robustamente perturbaciones pequeñas. \textcolor{red}{Esta definición es poco clara. ¿Cómo se mide específicamente?}
\end{itemize} 

En \cite{LEAF} se propone el framework \textit{LEAF}. \textcolor{red}{En este} se introducen 5\textcolor{red}{en LEAF se proponen 4 métricas y 1 restricción¡¡¡ la concisión. Relee el artículo para más detalles.} métricas para evaluar de forma clara las explicaciones de LLEs. Las métricas son las siguientes:

\textcolor{red}{Revisa este itemize completo, detallalo mejor. Si hablas de $f$ y $g$, asegúrate de definirlos  anteriormente.}
\begin{itemize}
    \item \textit{Conciseness: }se interpreta como hasta qué punto el modelo es entendible por un humano. Se define como el número de pesos de la explicación (es una LLE) \textcolor{red}{que} son no nulos. \textcolor{red}{Esta definición es confusa. ¿Te refieres a la proporción de características relevantes?}
    \item \textit{Local Fidelity: }mide \textcolor{red}{cómo el} modelo de caja blanca $g$ (la explicación) aproxima el comportamiento del modelo de caja negra $f$ para un ejemplo $x$ y su vecindad $N(x)$. Este se calcula con el \textit{F1-score} sobre dicha vecindad y dependerá por lo tanto de cómo se muestree \textcolor{red}{ésta}. 
    \item \textit{Local Concordance: }mide \textcolor{red}{cómo} $g$ aproxima a $f$ justo en el ejemplo $x$. Se mide como $max\{0, 1-|f(x)-g(x)|\}$.  
    \item \textit{Reiteration Similarity: }muchos métodos de LLEs utilizan aleatoriedad para generar sus explicaciones. Debido a esto\textcolor{red}{,} aplicar muchas veces un algoritmo para generar una explicación para un mismo ejemplo $x$ puede generar explicaciones distintas cada vez. Se propone esta métrica para medir la \textcolor{red}{similitud} del conjunto de explicaciones generadas. \textcolor{red}{Ésta} se calcula como la \textcolor{red}{similitud} de \textit{Jaccard} $J$ esperada entre dos explicaciones $g$ y $g'$. Es decir\textcolor{red}{,} $\mathbb{E}[J(g,g')]$.
    \item \textit{Prescriptivity: }mide en qué medida una explicación local puede usarse para modificar una instancia y cambiar su clasificación. Se calcula proyectando $x$ sobre la frontera de decisión del modelo explicativo $g$, y evaluando qué tan cerca está esa proyección del límite. Sea $x'$ dicha proyección\textcolor{red}{;} entonces se calcula como la pérdida \textit{hinge} normalizada de $f(x')-g(x')$. \textcolor{red}{Esta definición es muy técnica y poco clara. Necesita más explicación intuitiva.}
\end{itemize}

Finalmente, en \cite{REVEL} se introducen las métricas REVEL, que ya hemos introducido en \ref{subsec:revel}. Este framework propone cinco métricas al igual que LEAF. REVEL está compuesto de las mismas 5 métricas, cambiando \textit{Reiteration Similarity} por \textit{Robustness} que busca medir lo mismo. Sin embargo\textcolor{red}{,} REVEL cambia cómo se calcula cada una de las métricas introduciendo mejoras en todas ellas. Es por esto \textcolor{red}{que} vamos a utilizar en este trabajo estas métricas para medir la calidad de nuestras explicaciones. \textcolor{red}{Deberías explicar cuáles son específicamente las mejoras que introduce REVEL respecto a LEAF, no solo mencionarlas, sobretodo si son las que finalmente usas.}

\highlight{¿Debería ir una por una explicando las mejoras de las métricas REVEL respecto a LEAF?} \textcolor{red}{Si}



\subsection*{Métodos de regularización XAI}

Los métodos de regularización en XAI \cite{XSHIELD, EstadoArteMetodos}, que añaden un término regularizador a la función de pérdida, plantean una manera sencilla para modificar el comportamiento del modelo, favoreciendo criterios de mejora de explicabilidad. Estos son por construcción agnósticos al modelo, aunque algunos requieren que \textcolor{red}{éste} sea diferenciable, y sencillos de implementar. Por ello son uno de los métodos más estudiados en \textcolor{red}{las} técnicas de XAI. \textcolor{red}{Esta última frase es mentira, apenas se ha estudiado. Hay que quitarla.}

En \cite{RRR} se introduce \textit{Right for the Right Reasons} (RRR) que busca optimizar el razonamiento del modelo. En este trabajo el conjunto de datos $X$, \textcolor{red}{además de} estar etiquetado, debe proveer una máscara en la que se indique si cada dimensión de la entrada debe ser relevante o irrelevante para la decisión. En este trabajo las explicaciones se hacen mediante el gradiente respecto \textcolor{red}{al} logaritmo de la salida del modelo. \textcolor{red}{Me faltan los pros y contras de este método, sobretodo para descartarlo en nuestra experimentación. La contra es que necesita una etiquetación extra que no siempre está disponible o es demasiado costosa.}

En \cite{REGINPUTGRAD} se realiza un método similar al anterior, pero solucionan el problema de la anotación manual de los datos para determinar qué parte de las entradas son irrelevantes. En \textcolor{red}{éste} se generan perturbaciones de la entrada y se \textcolor{red}{calculan} sus mapas de atribución. El componente regularizador penaliza cambios significativos en las explicaciones. Este método busca que las explicaciones sean más estables y robustas.

En \cite{simpleregularizacion} se introduce \textit{Contextual Decomposition Explanation Penalization} (CDEP). \textcolor{red}{Éste} utiliza la diferencia absoluta entre la explicación producida y la explicación dada por un experto. Para ello, se emplea el método \textit{Contextual Decomposition} (CD) como técnica de atribución, que permite calcular explicaciones diferenciables durante el entrenamiento. De esta forma, se penaliza directamente en la función de pérdida cualquier discrepancia entre el razonamiento del modelo y el conocimiento previo. \textcolor{red}{Lo mismo, pros y contras}

Finalmente, en \cite{XSHIELD} se introduce X-SHIELD\textcolor{red}{,} el método del que partimos. Este método no tiene restricciones teóricas sobre la entrada, salida o tarea que realiza el modelo, \textcolor{red}{salvo} que el modelo sea diferenciable. \textcolor{red}{la aplicación de esta regularización} mejora el rendimiento en las métricas REVEL, lo que nos proporciona un método base con el que comparar nuestras propuestas. \textcolor{red}{X-SHIELD} también ha demostrado que es capaz de aumentar las métricas REVEL mientras que también mejora en muchos casos el rendimiento en clasificación del modelo.
...
\textbf{X-SHIELD} \cite{XSHIELD} es el método de regularización XAI con el que vamos a comparar los métodos propuestos a continuación. Además\textcolor{red}{,} nuestros métodos están basados en los mismos principios que este método\textcolor{red}{:} esconder al modelo las características menos relevantes y penalizar la diferencia entre la predicción original del modelo y la predicción sin dichas características. Todos los detalles \textcolor{red}{acerca} de \textcolor{red}{X-SHIELD} están en la subsección \ref{subsec:XSHIELD}. \textcolor{red}{Esta sección debería incluir la descripción completa de X-SHIELD, no solo una referencia a otra subsección. Es el método base de tu trabajo.}



\section{Mis métodos propuestos} \textcolor{red}{El título "Mis métodos propuestos" es muy informal para un TFM. Usar "Métodos propuestos" o "Metodología desarrollada".}
En las siguientes tres subsecciones voy a introducir \textcolor{red}{las} propuestas de métodos de regularización \textcolor{red}{desarrolladas en este trabajo}.

\subsection{FXShield} \textcolor{red}{Debería ser FX-SHIELD (con guión) para consistencia con el resto del documento.}

En \textbf{FX-SHIELD} (\textit{Feature X-SHIELD}) proponemos una regularización que ocluye \textcolor{red}{las} distintas características extraídas por la parte de red convolucional del modelo. Sea $x \in \mathbb{R}^F$ un dato de entrada\textcolor{red}{;} luego $F$ es el número de características de la entrada. Sea $f:\mathbb{R}^F \rightarrow \mathbb{R}^C$ la función que implementa el modelo de caja negra\textcolor{red}{,} siendo $C$ el número de clases del problema. En el caso de una red convolucional se tiene que $f(x) = l(c(x))$ \textcolor{red}{donde} $c:\mathbb{R}^F \rightarrow \mathbb{R}^{F_c}$ es la función que \textcolor{red}{implementan} las capas convolucionales de la red y $l:\mathbb{R}^{F_c} \rightarrow \mathbb{R}^C$ la función que implementan las capas no convolucionales (por ejemplo una o varias capas totalmente conectadas). En las expresiones anteriores\textcolor{red}{,} $F_c$ representa el número de características que extrae la red convolucional de las entradas (ver Figura \ref{fig:fxshield_esquema}). Por ejemplo, en la Tabla \ref{tab:EfficientNetB2_arquitectura} podemos ver que para EfficientNet-B2 se tiene que $F_c=1408$. \textcolor{red}{Esta introducción matemática es muy densa. Necesitas una explicación intuitiva del concepto antes de las fórmulas.} 
...
Para X-SHIELD las características de las entradas, si son imágenes, dependen de \textcolor{red}{cómo} se quieran definir. Hay varias posibilidades como tomar que cada píxel se corresponda \textcolor{red}{con} una \textcolor{red}{característica}, dividir la imagen \textcolor{red}{de entrada} en cuadrados del mismo tamaño o utilizar métodos para dividir la imagen de entrada en superpíxeles. En FX-SHIELD la toma de esta decisión no es necesaria porque independientemente de la imagen de entrada (incluso para distintos tamaños de \textcolor{red}{ésta}) siempre se va a obtener el mismo número $F_c$ de características extraídas por los bloques convolucionales. \textcolor{red}{Este párrafo necesita mejor conexión con el anterior. ¿Por qué es importante esta diferencia? ¿Cuál es la ventaja práctica?} 

\textcolor{red}{Notemos} como $z=c(x)$ las características intermedias. Ahora buscamos ocluir\textcolor{red}{ocultar mejor} las características menos importantes de $z$. La importancia que se le da a las distintas características se puede definir con la matriz de importancia $\mathcal{A} = \frac{\delta f}{\delta Z}(x) = \frac{\delta l}{\delta Z}(z)$. Para cada \textcolor{red}{característica} $i \in \{1,2,...,F_c\}$ vamos a definir la importancia de \textcolor{red}{ésta} como: \textcolor{red}{La notación es inconsistente. Usas $Z$ mayúscula y $z$ minúscula para lo mismo.}
...
Ya estamos en condiciones de definir el factor de regularización que se utiliza en este método. Para un ejemplo $x$ y unos parámetros $\Theta$ del modelo específicos, sean $y=f_\Theta(x)=l(z)$ la predicción original del modelo y $y'=l(T_{fxshield}(c_\Theta(x))) = l_\Theta(z')$ la predicción para el ejemplo con las características intermedias modificadas\textcolor{red}{;} entonces se tiene que:

\begin{equation}
FX-SHIELD(x,\Theta) = KL(y,y') + KL(y',y)
\end{equation}

\textcolor{red}{Donde} $KL$ es la divergencia de Kullback-Leibler que se utiliza para medir la distancia estadística entre las distribuciones de probabilidad entre los modelos. \textcolor{red}{¿Por qué usas KL simétrica? ¿Cuál es la justificación teórica para usar $KL(y,y') + KL(y',y)$? Ponle el nombre, que lo tiene. La justificación es que la KL es entre la variable real y la predicha. En nuestro caso, no tenemos certeza de que una sea mejor que la otra, por lo que usamos la simétrica.}
...
\subsection{FRShield} \textcolor{red}{Debería ser FR-SHIELD para consistencia.}

\textbf{\textcolor{red}{FR-SHIELD}} (\textit{Random FX-SHIELD}) es el mismo método que FX-SHIELD pero cambia en que las características que oculta se escogen de forma aleatoria sin tener en cuenta la importancia de \textcolor{red}{éstas}. Este método nos permite comparar FX-SHIELD con un método no informado. Además\textcolor{red}{,} el hecho de que se oculten las características de forma aleatoria puede ayudar al modelo a no depender únicamente de las características más importantes y que las explicaciones estén apoyadas en más características. Finalmente\textcolor{red}{,} este método se parece a \textit{Dropout}, lo que también lleva a pensar que podría hacer que la red no \textcolor{red}{sobreajuste}.

\subsection{HShield} \textcolor{red}{Debería ser H-SHIELD para consistencia.}

Finalmente, introducimos \textbf{H-SHIELD} (\textit{Hybrid} X-SHIELD)\textcolor{red}{,} un método que combina X-SHIELD con FX-SHIELD. Para ello modificamos el valor \textcolor{red}{regularizador} para tener en cuenta ambos de la siguiente forma:
...
\textcolor{red}{donde} $\alpha_1, \alpha_2$ son dos parámetros positivos que utilizaremos para \textcolor{red}{cambiar} el peso que le damos a cada una de las regularizaciones por separado. 

Con esta propuesta intentamos combinar las bondades de ambos métodos. El principal inconveniente de este método es su mayor coste computacional \textcolor{red}{respecto a} los métodos por separado. La determinación de los parámetros $\alpha_1$ y $\alpha_2$ se hará empíricamente comparando los valores de regularización individuales que se obtienen para cada método por separado. El porcentaje de elementos que se ocultan por cada método, que \textcolor{red}{denotábamos} como $\lambda$, ahora son dos parámetros distintos $\lambda_1$ y $\lambda_2$, que serán dicho parámetro para X-SHIELD y FX-SHIELD respectivamente.

\section{Implementación}

Para la realización de este TFM se ha partido del \href{https://github.com/isega24/SHIELD/tree/X-SHIELD}{código} correspondiente al proyecto de X-SHIELD original \cite{XSHIELD}. \textcolor{red}{Éste} está escrito en \textit{Python}. Se ha realizado un estudio de todo el código \textcolor{red}{del} proyecto original para entender correctamente \textcolor{red}{cómo} funciona y así poder implementar los nuevos métodos que proponemos. Además se han escrito varios \textit{scripts} en \textit{bash} para la ejecución de los entrenamientos de los modelos aplicando los distintos métodos de regularización en los servidores de GPUs de la UGR.

A continuación, hacemos un resumen de todos los módulos del proyecto que se han utilizado, sin contar los \textit{scripts} de \textit{bash} \textcolor{red}{de ejecución}, y mencionamos \textcolor{red}{cuándo} se han modificado para el TFM: \textcolor{red}{Esta lista de archivos es demasiado técnica para el capítulo de Métodos. Pertenece a un anexo o al capítulo de Experimentos.}

\begin{itemize}
\item \textbf{SHIELD/shield.py:} \textcolor{red}{éste} es el módulo más importante de todo. En él vienen las funciones para calcular las importancias de las características y las implementaciones de los distintos métodos. En particular, se ha añadido a este archivo los métodos \textcolor{red}{FX-SHIELD y FR-SHIELD}.

\item \textbf{procedures/cargar\_datos.py:} este módulo es completamente nuevo y es necesario para cargar los datos del TFM en un \textit{dataset} de \textit{pytorch}. Este módulo se basa en la manera en la que se cargan los \textit{datasets} utilizados en el trabajo original de X-SHIELD, cuya \textcolor{red}{implementación} se puede ver en el \href{https://github.com/ari-dasci/S-ReVel}{GitHub} de las métricas REVEL.

\item \textbf{procedures/procedures.py:} aquí vienen implementadas las funciones para cargar el modelo clasificador (\textit{EfficientNet} en nuestro caso) y las funciones que realizan un paso \textcolor{red}{de} entrenamiento y validación.
...
\textcolor{red}{Texto introductorio del capítulo. Explica las diferentes secciones que hay y qué se va a ver en cada una, una frase por sección aproximadamente.}
...
\textcolor{red}{Esto es porque no está publicado. Debes comentar lo relevante del documento en el texto y decir que es un documento interno. También puedes comentar que se está trabajando en su publicación.}
...
\caption{En esta tabla se muestra la clasificación de los tejidos, una breve descripción de qué significan y si el tejido es maligno.\textcolor{red}{Clasificación, descripción y malignidad de los tejidos presentes en el conjunto de datos} Tabla extraída y adaptada de \protect\cite{datosObtencion}.} \label{tab:datasetClases} 
\end{longtable}
...
\caption{Imagen de tejido segmentado. Extraída de \protect\cite{datosObtencion}.}
...
Además los datos están divididos en dos carpetas\textcolor{red}{conjuntos bien diferenciados, carpetas no es el término adecuado} \textit{train} y \textit{val}. El conjunto de \textit{train} contiene 113.491 imágenes ($~90\%$ de los datos) mientras que \textit{val} contiene las 12.618 imágenes restantes ($~10\%$ de los datos). Esta partición de los datos se realizó para entrenamiento/test de los modelos y vamos a respetarla.\textcolor{red}{no es respetar, es que así nuestros experimentos podrán ser reproducible sporque no nos hemos inventado nosotros la partición que queramos} Se puede comprobar que esta partición de los datos mantiene la proporción\textcolor{red}{estratificada} de las clases como se puede ver en la Figura \ref{fig:distribucionDatosTrainTest}.
...
\textcolor{red}{Yo quitaría la parte donde la comentas arriba y mantendría esta subsección, pero no repitas lo mismo dos veces.}
...
\textcolor{red}{Este párrafo queda raro, hablas mucho de 90/10. En lugar de comparar con 80/20, habla directamente de que esta partición es la que tenemos y que es adecuada dado el tamaño del dataset. Otra cosa, lo de que sea 'muy importante' no suena bien. Es una práctica estándar y se hace para tal y tal, la importancia no es tan científico.}

\paragraph*{Validación}

Teniendo en cuenta el tamaño de nuestro conjunto de datos y de los recursos computacionales de los que disponemos hemos decidido utilizar un conjunto de validación fijo sobre otros métodos como \textit{k-fold cross validation}.\textcolor{red}{No hables de lo que no usamos, la validación nos vale porque tenemos muchos datos y recursos limitados, no hace falta mencionar k-fold. Si te lo preguntan en la presentación ya lo comentas, pero tenemos que remarcar lo que si has hecho.} 


Esta técnica consiste en separa un porcentaje del conjunto de entrenamiento para no entrenar con él. Sin embargo se va a utilizar para evaluar el modelo al final de cada época, calculando su función de error sobre este conjunto. Esta técnica es importante \textcolor{red}{lo mismo que la importancia que he dicho antes, di lo que consigue, la importancia es algo subjetivo} para el entrenamiento del modelo porque que cumple un rol distinto a los conjuntos de entrenamiento y test. Este nos ayudará a realizar un ajuste de hiperparámetros del modelo y ayuda a prevenir el sobreajuste, eligiendo el modelo que menor error tenga en validación, en vez de en entrenamiento. 
...
Cómo \textbf{\textit{learning rate}} inicial Adam se ha escogido el valor $0.0004$. Un \textit{learning rate} inicial es muy importante\textcolor{red}{determinante, otro adejetivo que no sea importante} porque dicta como converge el modelo. Para la elección de este parámetro se han hecho pruebas iniciales sobre los modelos y hemos determinado que este valor es el que mejor convergencia daba al modelo.

El tamaño del \textbf{\textit{minibatch}} escogido ha sido 32. Está demostrado\textcolor{red}{justifica con cita: en tal trabajo[?] se ve...} que tamaños grandes de \textit{minibatches} acelera el entrenamiento y estabiliza la convergencia del modelo. Por otro lado un tamaño pequeño puede mejorar la capaz de generalizar del modelo \cite{batchSize}. El tamaño suele escogerse como una potencia de $2$\textcolor{red}{aclaración innecesaria}. Hemos escogido tamaño $32$ ya que para $64$ la gráfica dónde ejecutamos los experimentos se quedaba sin memoria y empíricamente se comprobó que $32$ daba mejores resultados que tamaños menores\textcolor{red}{Di simplemente que has escogido 32 por la cita que antes has tenido que buscar y porque más grande no cabía, el resto no es necesario}. 

Para el número de \textbf{épocas}, hemos escogido un número no muy alto\textcolor{red}{Di el numero de épocas, no muy alto es subjetivo. Dependiendo del dataset, podría ser suficiente con 10 épocas o podría necesitar 50.} ya por los recursos computacionales de los que disponemos. Por ello, se hicieron pruebas experimentales previas para buscar un valor que garantizaba la convergencia de estos pero no se excedía en tiempo de entrenamiento. El número de épocas es por lo tanto $24$. Con este número de épocas los modelos entrenados con la regularización X-SHIELD (la más costosa) tardan alrededor de 1 día (normalmente un poco más)\textcolor{red}{aclaración innecesaria} en entrenarse.\textcolor{red}{reescribir mejor esta frase}

Cómo ya sabemos el modelo que vamos a utilizar en los experimentos es EfficientNet-B2 con entradas de tamaño $224 \times 224$\textcolor{red}{Esta frase está incompleta}. Este modelo lo cargamos de la librería de \textit{pytorch}. Además lo vamos a cargar con los pesos preentrenados de esta red sobre el conjunto de datos IMAGENET-1K lo que se conoce como \textit{transfer learning}\textcolor{red}{no recuerdo si has mencionado antes el transfer learning en los preliminares, pero deberías para que no sea esta la primera vez que se menciona} y nos permite no tener que entrenar el modelo desde cero\textcolor{red}{Esto lo que nos permite es comenzar el entrenamiento con un modelo cuyas características ya tienen cierto entrenamiento}.

\textcolor{red}{Creo que puedes resumir todo lo anterior }

En primer lugar, vamos a entrenar el modelo EfficientNet-B2 con los parámetros que hemos especificado. Después se entrenará esta red aplicando los distintos métodos de regularización explicados, estos son: el método original con el que comparamos los propuestos (X-SHIELD) y los métodos nuevos que proponemos en este trabajo (FX-SHIELD, FR-SHIELD y H-SHIELD).
...
Para elegir los mejores hiperparámetros para cada método se escoge el modelo entrenado que minimice la \textit{accuracy} en validación para dichos hiperparámetros.


\section{Métricas}

Para el entrenamiento del modelo se utiliza como función de pérdida el \textbf{error de entropía cruzada} que es la función de pérdida que se utiliza habitualmente para entrenar redes \textcolor{red}{el apellido neuronales hay que ponerselo} en clasificación.

Para evaluar el rendimiento del modelo en la tarea de clasificación utilizamos la \textcolor{red}{métrica de, si no es una cacofonía} \textbf{\textit{accuracy}}, que mide la proporción de ejemplos correctamente clasificados. Esta toma valores en $[0,1]$ y cuanto mayor es, mejor es el rendimiento del modelo. \textcolor{red}{primera vez que lo explicas, deberías explicarlo antes.}

Por otro lado para evaluar el rendimiento utilizamos la métricas \textbf{REVEL}. Estas fueron formalmente introducidas en \ref{subsec:revel}\textcolor{red}{No hace falta que las recuerdes, ya has mencionado donde están explicadas}. Recordamos que estas son: \textit{local concordance}, \textit{local fidelity}, \textit{presciptivity}, \textit{conciseness} y \textit{robustness}. Para cada método de regularización, las calcularemos únicamente para el modelo final que ha sido calculado\textcolor{red}{usas mucho calcular, cambiar términos (obtenido quizas?)} para los mejores parámetros para\textcolor{red}{de, cacofonía} dicho método.

Para el cálculo de estas métricas\textcolor{red}{coma} se deben crear\textcolor{red}{se generan} explicaciones con un \textcolor{red}{generador de} LLEs. Elegimos \textbf{LIME}\textcolor{red}{Elegimos LIME por los resultados en métricas obtenidos en el artículo de REVEL que obtiene mejores explicaciones que SHAP}. Utilizamos las mismas características que X-SHIELD para LIME, es decir \textcolor{red}{coma}64 cuadrados del mismo tamaño sobre la imagen de entrada. Para los cálculos de la explicación de un ejemplo,\textcolor{red}{utilizamos la configuración de REVEL:} generamos 1000 vecinos. El parámetro $\sigma$ (explicado en la subsección \ref{subsec:LIME}\textcolor{red}{ya lo has comentado, aclaración innecesaria}) lo ponemos a 12.

Para cada ejemplo\textcolor{red}{coma} generamos 10 explicaciones para el cálculo de las cuatro primeras métricas y calculamos la media aritmética de cada una para obtener su valor final para ese ejemplo. En total, utilizamos 100 ejemplos. Para el cálculo de la \textit{robustness} utilizan las 10 explicaciones generadas para las otras métricas para calcular esta métrica. Hemos elegido estos parámetros para calcular estas métricas por límites de coste computacional. El cálculo de estas métricas para un modelo ya entrenado tarda alrededor de 1 día.


\paragraph*{Test estadísticos bayesianos}

Una vez tenemos los 100 valores de las métricas REVEL para cada método utilizamos la librería de \textit{Python} \href{https://baycomp.readthedocs.io/en/latest/\#baycomp}{baycomp}\textcolor{red}{No utilizamos tal librería que tiene tal test. Aplicamos tal test, el cual encontramos implementado en tal librería }. Esta biblioteca introduce los tests bayesianos para comparación de método. Estos tests permiten comparar dos métodos de manera probabilística, estimando directamente la probabilidad de que uno sea mejor que otro, o que la diferencia entre ellos sea insignificante\textcolor{red}{significativamente relevante}.

Los tests de baycomp\textcolor{red}{los test bayesianos, no baycomp. Esa es la librería} se basan en la estimación de la distribución posterior de la diferencia de desempeño entre dos métodos. Se calculan las tres probabilidades siguientes:
\begin{itemize}
...
\textcolor{red}{Párrafo introductorio de la sección. Explica las subsecciones que hay y qué se va a ver en cada una, una frase por subsección aproximadamente.}

\subsection{Entrenamiento y accuracy}

\textcolor{red}{Párrafo introductorio}

Los resultados de todas las redes entrenadas con los distintos métodos y para todas las combinaciones de hiperparámetros se encuentran en la Tabla \ref{tab:resultadosTodo}. En esta se puede ver que hiperparámetros son los mejores para cada modelo\textcolor{red}{ Lo mismo, ser el mejor es subjetivo, di que tiene mejor rendimiento en tal métrica, como dices después}, es decir, con qué parámetros han conseguido la mejor \textit{accuracy} en el conjunto de validación. 
...
\textcolor{red}{Me hace falta un párrafo que extraiga algo más interesante que decir cuál es mejor. Hay algún patrón en los resultados? Aumentar lambda empeora o mejora el rendimiento? FR-SHIELD empeora mucho con lambda grande, por qué puede ser?}
...
FR-SHIELD ($\lambda = 77.0$) & 0.930 & 0.02603 & 0.011845 & 0.933 & 0.02600 & 0.011897 \\
\hline
...
\textcolor{red}{Hay experimentos dispares por cada método, no? Deberías comparar primero los experimentos de cada método entre sí y justificar por qué se han hecho mas de uno que de otro. Por qué más de FX-SHIELD y pocos de H-SHIELD? Luego comparar los mejores de cada método. }

\newpage

En la Tabla \ref{tab:resultadosMejoresTest} tenemos los valores obtenidos en test para los mejores modelos de cada método. Cómo esperábamos\textcolor{red}{son resultados, no debemos esperar nada} el método X-SHIELD mejora la \textit{accuracy} del modelo \textit{baseline}, en este caso en 0.07. Luego es una mejora significativa. Por otro lado el método híbrido H-SHIELD consigue capturar está mejoría, empeorando únicamente en un 0.01 su \textit{accuracy} respecto de X-SHIELD. Por otro lado los métodos FX-SHIELD y FR-SHIELD no han conseguido mejorar al modelo base, obteniendo ambas 0.949 de \textit{accuracy}, prácticamente la misma que el modelo base (un 0.01 de diferencia a favor del modelo base)\textcolor{red}{Esto lo has dicho varias veces no?}. Observemos también que H-SHIELD tiene con diferencia el mayor valor de su función de regularización en test a pesar de ser la suma de X-SHIELD y FX-SHIELD. Esto parece indicar que \textcolor{red}{nombre del método al que hace referencia} no es capaz de minimizar ambos valores de regularización al mismo tiempo.
...
En la Figura \ref{fig:metrics_xshield} se encuentran las curvas de aprendizaje de X-SHIELD para sus distintos parámetros. Se observa un comportamiento natural de convergencia de la red\textcolor{red}{: la función de pérdida en entrenamiento continúa disminuyendo mientras que, en comparativa, la de validación se estabiliza}, podría parecer que podría incluso mejorar un poco más viendo la gráfica del error en entrenamiento, aunque la de validación si parece que ha convergido. \textcolor{red}{No es que sea interesante, 'Cabe destacar' o 'Notamos' mejor}Es interesante ver que aunque en validación los valores de regularización siguen un orden y parecen converger a valores distintos (mayor cuanto mayor es el $\lambda$) lo cual es esperable, en entrenamiento estos convergen a valores muy cercanos.
...
Por otro lado, en las curvas de aprendizaje de FX-SHIELD (Figura \ref{fig:metrics_fxshield}) se observa un comportamiento normal\textcolor{red}{textPor otro lado? pero si es lo mismo que antes. Escoge otro conector. 'Al igual que...'}. Al igual que ocurría con X-SHIELD, sus valores de regularización convergen todos a valores muy cercanos en entrenamiento. Por otro lado, y esto es más sorprendente\textcolor{red}{subjetivo, innecesario}, esto también ocurre con sus valores de regularización. Podemos ver que la función de pérdida en entrenamiento converge correctamente y en validación este método necesita menos épocas para aprender ya que el valor en validación de la pérdida empieza a aumentar ligeramente (aunque la \textcolor{red}{métrica de precisión} accuracy en validación sigue mejorando). Esto no afecta a nuestros resultados porque el modelo final es el que mejor pérdida obtuvo en validación, no el de la última época\textcolor{red}{Lo malo de esto es que puede ser que estos resultados mejoren si continuamos entrenando el modelo...}.

\textcolor{red}{En la Figura \ref{fig:metrics_fxshield}, observamos la evolución de las métricas para el modelo FX-SHIELD. Se observa...}
...
\textcolor{red}{Se confirma lo que te comento, parece que todavía se puede exprimir con más épocas de entrenamiento H-SHIELD y X-SHIELD.}
...
\textcolor{red}{Párrafo introductorio. Explica el objetivo de la subsección y qué se va a ver.}
...
De los \textit{violin plots}\textcolor{red}{Es un gráfico de violín, menos anglicismos} de la \textit{conciseness}\textcolor{red}{concisión, lo mismo} no se puede discernir nada con una inspección visual excepto que la distribución para FR-SHIELD es muy distinta a la de los otros métodos.\textcolor{red}{habla más en positivo, di lo que se puede ver, no lo que no se puede ver.}

Por último,\textcolor{red}{esta coma, como otras, si que estaba. Como esta, todas} para la \textit{prescriptivity}\textcolor{red}{prescriptividad} se tienen dos distribuciones parecidas para X-SHIELD y H-SHIELD, estando la de X-SHIELD desplazada hacia arriba\textcolor{red}{valores altos}. Por otro lado se tienen la de FX-SHIELD y FR-SHIELD que son parecidas también. A simple vista, no es fácil ver cual de las dos podría ser mejor\textcolor{red}{No podemos cerciorarnos de si hay diferencias significativas entre ambas distribuciones}.
...
Los últimos resultados obtenidos vienen de los test bayesianos que hemos realizado sobre estas métricas. En la Tabla \ref{tab:bay-X-FX} se tiene que para la \textit{local concordance}\textcolor{red}{concordancia local} y la \textit{local fidelity} de ambos\textcolor{red}{los métodos X-SHIELD y FX-SHIELD. 'Ambo's se usa cuando se usan dos y solo dos elementos, no si después los tienes que especificar } métodos (X-SHIELD y FX-SHIELD) son iguales. Además FX-SHIELD mejora la \textit{prescriptivity} y la \textit{conciseness}, aunque esta última sin una diferencia estadísticamente significativa. De la misma forma, X-SHIELD presenta una mejor \textit{robustness}.
...
\textcolor{red}{En los tests bayesianos, la terminología correcta es no es que coincidan, sino que 'no podemos establecer diferencias significativas entre tales métodos en tales métricas'.}
...
\bottomrule
\end{tabular}
\caption{Resultados de los tests bayesianos entre XShield y FXShi\textcolor{red}{Esto lo has dicho varias veces no?}eld.}
\label{tab:bay-X-FX}
\end{table}
...
\textcolor{red}{Por mi parte, la sección de discusión es la de conclusiones y trabajos futuros. Los detalles y conclusiones técnicas que planteas aquí deberían ir en la sección anterior como conclusiones experimentales. Intenta separar entre conclusiones técnicas en la sección anterior, y conclusiones más generales y hacer una sección final potente.}
...
En resumen, el método propuesto FX-SHIELD es una versión mejor de FR-SHIELD (otro método propuesto)\textcolor{red}{aclaración innecesario. Remarca que es la versión aleatoria, no que es 'otro' y ya está} lo cual era de esperar\textcolor{red}{era de esperar queda raro, di las diferencias y explica por qué, no presupongas que el lector lo va a esperar al igual que nosotros} porque FR-SHIELD es una versión aleatoria (menos informada)\textcolor{red}{los paréntesis intenta quitarlos siempre} que FX-SHIELD. Por otro lado FX-SHIELD produce explicaciones más interpretables que X-SHIELD, aunque menos robustas. Por otro lado, X-SHIELD mejora el rendimiento del modelo en la tarea de clasificación, mejorando la \textit{accuracy} en test. Nuestro último método propuesto H-SHIELD, que combina los métodos X-SHIELD y FX-SHIELD consigue capturar las ventajas de X-SHIELD (mejor \textit{accurcy} y mayor \textit{robustness}) pero no es capaz de mejorar a FX-SHIELD en el resto de métricas REVEL, sin embargo, es capaz de mejorar la robustez de las explicaciones respecto al método X-SHIELD. Además, todos los métodos con los que hemos experimentado aseguran un mínimo de calidad en sus explicaciones al tener las métricas de \textit{local fidelity} y \textit{local concordance} prácticamente perfecta\textcolor{red}{no uses perfecta como adjetivo, no queda bien. Lo que dices después si es lo que querías decir, está bien ->}, con distribuciones muy concentradas alrededor del máximo.

\clearpage
\chapter{Conclusiones} \label{cap:conc}

En este trabajo nos hemos centrado en el problema de mejorar la interpretabilidad de modelos de clasificación de lesiones cancerosas en biopsias de próstata utilizando técnicas de IA explicable. Para el desarrollo de este\textcolor{red}{,} lo dividimos en objetivos más pequeños que hemos cumplido a lo largo del trabajo. En primer lugar\textcolor{red}{,} realizamos una revisión del estado del arte de los distintos métodos necesarios para abordar el problema principal: modelos de clasificación, \textcolor{red}{métodos} de XAI para producción\textcolor{red}{generación} de explicaciones, métodos XAI \textcolor{red}{para la} mejora de explicaciones y métricas XAI que midan \textcolor{red}{aspectos cuantitativos de}cuantitativamente los distintos métodos propuestos. A continuación\textcolor{red}{,} realizamos un estudio del código de la regularización X-SHIELD \cite{XSHIELD}, método del que parten los que proponemos nosotros, y las métricas REVEL \cite{REVEL}\textcolor{red}{,} que son las que utilizamos para comparar los distintos métodos. Después\textcolor{red}{,} propusimos \textcolor{red}{presentamos} en detalle nuestros métodos originales: FX-SHIELD, FR-SHIELD y H-SHIELD, \textcolor{red}{tres regularizaciones de la familia T-SHIELD}. Tras esto\textcolor{red}{,} los implementamos\textcolor{red}{se sobreentiende que los implementaste} y diseñamos los distintos experimentos\textcolor{red}{Creo que esto no aporta nada, habla más de 'comprobar' su potencial, este estudio es para el que lea la sección experimental}: el modelo \textcolor{red}{de} clasificación utilizado, detalles de los hiperparámetros utilizados, explicación del conjunto de datos utilizado y cómo lo hemos \textcolor{red}{dividido}, y métricas y \textcolor{red}{tests} estadísticos utilizados para comparar los distintos métodos. Finalmente\textcolor{red}{,} \textcolor{red}{ejecutamos} estos\textcolor{red}{experimentos}, evaluamos los resultados obtenidos y discutimos su significado.


Nuestro método FX-SHIELD mejora al método del estado del arte X-SHIELD en dos de las cinco métricas REVEL, consigue el mismo rendimiento en otras dos \textcolor{red}{métricas} y empeora levemente la \textit{robustness}. Además\textcolor{red}{,} FX-SHIELD mantiene la \textit{accuracy} obtenida por el modelo base. Podemos afirmar que FX-SHIELD \textcolor{red}{produce mejores} explicaciones que X-SHIELD\textcolor{red}{. Esta afirmación es muy categórica considerando que solo mejora en 2 de 5 métricas. Sería más preciso decir que "FX-SHIELD muestra ventajas en interpretabilidad" o algo similar}. Por otro lado\textcolor{red}{,} también hemos demostrado\textcolor{red}{no hemos demostrado nada matemático, otro verbo} que FX-SHIELD \textcolor{red}{es} estrictamente mejor \textcolor{red}{muestra un mejor comportamiento, estrictamente mejor no tiene sentido aqui}que FR-SHIELD\textcolor{red}{,} pues para cada métrica REVEL lo mejora o \textcolor{red}{iguala}, destacando lo importante que es que la\textcolor{red}{s} características enmascaradas por FX-SHIELD sean las de menor importancia. Nuestro último método\textcolor{red}{,} H-SHIELD\textcolor{red}{,} que combina FX-SHIELD y \textcolor{red}{X-SHIELD,} tiene un rendimiento parecido a X-SHIELD en las métricas REVEL y \textcolor{red}{también} en \textit{accuracy}\textcolor{red}{. La redacción "como en accuracy" es confusa}. 

Por todo lo expuesto anteriormente\textcolor{red}{,} podemos afirmar que se ha conseguido el objetivo final del TFM\textcolor{red}{,} pues hemos presentado un método de regularización XAI \textcolor{red}{novedoso} que mejora las explicaciones obtenidas en las métricas REVEL respecto al método del estado del arte y lo hemos aplicado a un modelo de clasificación de lesiones cancerosas en biopsias de próstata para mejorar su interpretabilidad\textcolor{red}{. Esta conclusión podría ser más matizada - no mejora en todas las métricas, sino en algunas. Yo diría que encamina las posibles mejoras posteriores.}.
...
Destacamos a continuación los posibles trabajos futuros que consideramos más relevantes y prometedores. En primer lugar\textcolor{red}{,} sería interesante probar FX-SHIELD y X-SHIELD con distintos conjuntos de datos, elecciones de modelos y LLEs para aportar más pruebas a la superioridad de las explicaciones producidas por nuestro método\textcolor{red}{. Esta afirmación presupone la superioridad, que no está completamente demostrada}. En segundo lugar\textcolor{red}{,} \textcolor{red}{sería valioso} estudiar otras formas más sofisticadas de combinar FX-SHIELD y X-SHIELD \textcolor{red}{que el método H-SHIELD} propuesto para intentar conseguir que las bondades que presentan ambos se combinen\textcolor{red}{. La redacción "al método propuesto" es confusa, di a cuál te refieres}. Por ejemplo\textcolor{red}{,} un método que \textcolor{red}{calcule} la importancia de las características de la entrada e intermedias, tras lo que \textcolor{red}{realice} un paso por la red del dato perturbado \textcolor{red}{ocultando} también las características intermedias. Finalmente\textcolor{red}{,} proponemos buscar mecanismos que aumenten la generalización del modelo cuando se \textcolor{red}{entrene} con FX-SHIELD para \textcolor{red}{desarrollar un método que} mejore interpretabilidad y poder de clasificación simultáneamente.
