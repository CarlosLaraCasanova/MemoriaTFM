...
En los últimos años, el creciente uso de técnicas de \textbf{Inteligencia Artificial (IA)} en el campo de la salud ha conllevado mejoras significativas en el diagnóstico de PCa \cite{Rabaan2022}. En el campo de la IA, la necesidad de asegurar transparencia y confiabilidad ha dado pie a acuñar el término de \textbf{Inteligencia Artificial eXplicable} (\textit{eXplainable Artificial Inteligence, XAI}) \cite{XAIManifesto}, que es un campo que busca mejorar la interpretabiliad de los modelos de \textbf{Machine Learning (ML)}. El uso de XAI es especialmente importante en ámbitos donde la toma de decisiones repercute directamente en la vida de las personas, como es el caso de la medicina. La Unión Europea ha propuesto una \href{https://eur-lex.europa.eu/eli/reg/2024/1689/oj}{regulación} para el uso de IA asistida en estos aspectos, entre ellos, que estos sistemas sean capaces de explicar sus decisiones de forma clara y comprensible.
...
\textcolor{red}{Por otra parte, haría varios cambios en la estructura de los fundamentos teóricos:}
\begin{itemize}
    \item \textcolor{red}{Primero, }
    \item \textcolor{red}{Segundo, }
\end{itemize}
...
Para este TFG\textcolor{red}{Ciudado, es TFM no TFG} es relevante el aprendizaje supervisado. En particular serán relevantes los dos siguientes problemas típicos de aprendizaje supervisado.\textcolor{red}{ No es que sea relevante, es que es el enfoque que más se usa en el trabajo. Destacamos supervisado porque tenemos la etiqueta costosa, si no, no podríamos.}

\textcolor{red}{Falta párrafo resumen de las subsecciones que cuelgan a continuación.}
...
Las \textbf{Explicaciones lineales locales} (\textit{Linear Local Explanation, LLE}) \cite{REVEL,LIME} son una clase de explicaciones para modelos de caja negra.
 ...
 Sea $X \subset \mathbb{R}^F$ el datatset\textcolor{red}{dataset} de entrada, siendo $F$ el número de características por ejemplo, y sea $f : \mathbb{R}^F \rightarrow \mathbb{R}^C$ la función que implementa un modelo de caja negra, dónde\textcolor{red}{donde} $C$ es el número de clases de un problema de clasificación al que se aplica. Sea $x\in X$ una entrada a ser explicada. Un explicador LLE de caja blanca es una función $g: \mathbb{R}^F \rightarrow \mathbb{R}^C$ definida como:
...
Este Trabajo de Fin de Máster trata de resolver \textcolor{red}{abarcar} el problema de construir un modelo de IA de clasificación para lesiones cancerosas en biposias \textcolor{red}{biopsias (error ortográfico)} de próstata y de la mejora en interpretabilidad\textcolor{red}{explicabilidad} del modelo mediante técnicas de XAI. Por lo tanto vamos a hacer una revisión del estado del arte para los cuatro siguientes puntos \textcolor{red}{que son relevantes (frase incompleta)}:
...
% CORRECCIONES ORGANIZATIVAS - CAPÍTULO 5: EXPERIMENTOS
% =====================================================
% PROBLEMAS IDENTIFICADOS:
% - Diseño experimental mezclado con resultados
% - No hay justificación de decisiones experimentales
% - Configuración experimental dispersa por el capítulo
% - Falta análisis de potencia estadística
%
...
%
% MEJORAS NECESARIAS PARA ESTE CAPÍTULO:
% - Justificar elección de hiperparámetros
% - Explicar división train/test/validation
% - Describir protocolo experimental completo
% - Establecer métricas de éxito a priori
% - Incluir análisis de potencia estadística para tests bayesianos
...
\caption{En esta tabla se muestra la clasificación de los tejidos, una breve descripción de qué significan y si el tejido es maligno. Tabla extraída y adaptada de \protect\cite{datosObtencion}.} \label{tab:datasetClases} 
\end{longtable}
...
\caption{Imagen de tejido segmentado. Imagen extraída de \protect\cite{datosObtencion}.}
...
Nuestro método FX-SHIELD mejora al método del estado del arte X-SHIELD en dos de las cinco métricas REVEL, consigue el mismo rendimiento en otras dos de métricas y empeora levemente la \textit{robustness}. Además FX-SHIELD mantiene la \textit{accuracy} obtenida por el modelo base. Podemos afirmar que FX-SHIELD método produce mejores explicaciones que X-SHIELD. Por otro lado también hemos demostrado que FX-SHIELD en estrictamente mejor que FR-SHIELD pues para cada métrica REVEL lo mejora o es igual, destacando lo importante que es que la características enmascaradas por FX-SHIELD sean las de menor importancia. Nuestro último método H-SHIELD que combina FX-SHIELD y H-SHIELD tiene un rendimiento parecido a X-SHIELD en las métricas REVEL y como en \textit{accuracy}. 

Por todo lo expuesto anteriormente podemos afirmar que se ha conseguido el objetivo final del TFM pues hemos presentado un método de regularización XAI novel que mejora las explicaciones obtenidas en las métricas REVEL respecto al método del estado del arte y lo hemos aplicado a un modelo de clasificación de lesiones cancerosas en biopsias de próstata para mejorar su interpretabilidad.
...
Destacamos a continuación los posibles trabajos futuros que consideramos más relevantes y prometedores. En primer lugar sería interesante probar FX-SHIELD y X-SHIELD con distintos conjuntos de datos, elecciones de modelos y LLEs para aportar más pruebas a la superioridad de las explicaciones producidas por nuestro método. En segundo lugar estudiar otras formas más sofisticadas de combinar FX-SHIELD y X-SHIELD al método propuesto para intentar conseguir que las bondades que presentan ambos se combinen. Por ejemplo un método que calcula la importancia de las características de la entrada e intermedias, tras lo que hace un paso por la red del dato perturbado y ocultando también las características intermedias. Finalmente proponemos buscar mecanismos que aumenten la generalización del modelo cuando se entrenen con FX-SHIELD para expandir este y que mejore interpretabilidad y poder de clasificación simultáneamente.
